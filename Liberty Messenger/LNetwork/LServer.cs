...
namespace LNetwork
{
    public class LServer
    {
        ...
        private void ServerLoop()
        {
            ...
            while (this.stayConnected)
            {
                if (this.listener.Pending())
                {
                    ...
                    foreach(LClient lc in userNames)
                    {
                        if (lc.Client == client)
                        {
                            lc.LoadClient(client);
                        }
                    }
                    //LClient user = new LClient(client);
                }
        ...
        /// <returns></returns>
        public static ELoginError ConnectAsUser(LClient client)
        {
            MessageConnect user = (MessageConnect)client.Info;
            //test if user even exists first
            if (userNames.ContainsKey(user.User))
            {
                //test the password with the username to see if it is that user
                if ((string)userPassword[user.User] != user.Pass)
                    return ELoginError.BadPass;

                //test if the nickname is already in use by another user
                if (userNickNames.ContainsKey(user.Nick))
                {
                    //if the assigned nickname isn't for the user then returns in use.
                    if ((string)userNickNames[user.Nick] != user.User)
                    {
                ...
                userNickNames.Add(user.Nick, user.User);
                return ELoginError.None;
            }

            //create user from scratch since no user name was found
            userNames.Add(user.User, client);
            userNickNames.Add(user.Nick, user.User);
            userPassword.Add(user.User, user.Pass);
